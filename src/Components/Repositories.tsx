...
const Repositories = () => {
    ...
    if (isLoading) {
        return <><div role="status">
            <svg aria-hidden="true" className="w-8 h-8 mr-2 mb-16 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    if (error) {
        return <div className='flex justify-center my-6'><h1>En estos momentos estamos trabajando para mostrarte esta sección!</h1></div>
    }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-16">
                    {data && data.length > 0 ? (
                        data.map((repo) => (
                            <div
                                key={repo.id}
                                className={`p-4 border rounded-lg shadow-md bg-white dark:bg-gray-800 ${hoveredRepoId === repo.id ? 'bg-blue-200' : ''
                                    }`}
                            ...
                            >
                                ...
                                <a className="text-gray-600 dark:text-gray-400" href={repo.html_url}>{repo.url}</a>
                            </div>
                        ))
                    ) : (
